...
exports.handler = async (event) => {
  ...
  let interviews, incentive, termsAccepted, termsAcceptedAt;
  try {
    const body = JSON.parse(event.body || '{}');
    interviews = parseInt(body.interviews, 10);
    incentive = parseInt(body.incentive, 10);
    termsAccepted = body.termsAccepted === true;
    termsAcceptedAt = typeof body.termsAcceptedAt === 'string' ? body.termsAcceptedAt : null;
  } catch (e) {
  ...
  if (!termsAccepted) {
    return { statusCode: 400, body: JSON.stringify({ error: 'Terms & Conditions must be accepted before checkout.' }) };
  }
